...
if ($_SERVER['REQUEST_METHOD'] === 'POST' && ($_POST['action'] ?? '') === 'update_recurring') {
    ...
    if (!$rec) { echo json_encode(['error' => 'Not found']); exit(); }
...
if ($_SERVER['REQUEST_METHOD'] === 'POST' && ($_POST['action'] ?? '') === 'delete_recurring_permanently') {
    ...
    $stmt->execute(['id' => $recurringID, 'uid' => $userID]);
    if (!$stmt->fetch()) { echo json_encode(['error' => 'Not found or not cancelled yet']); exit(); }
...
<head>
    ...
    <style>
        .recurring-card { border-left: 4px solid #22c55e; transition: box-shadow 0.2s; position: relative; }
        .recurring-card:hover { box-shadow: 0 4px 16px rgba(0,0,0,.08); }
        .status-badge-active    { background:#dcfce7; color:#16a34a; }
        .status-badge-paused    { background:#fef9c3; color:#854d0e; }
        .status-badge-cancelled { background:#fee2e2; color:#991b1b; }
        .freq-btn.active { background:#198754; color:#fff; border-color:#198754; }
        .item-qty-input { width: 65px; text-align:center; }
        .next-delivery-chip { background:#f0fdf4; border:1px solid #bbf7d0; border-radius:20px; padding:2px 12px; font-size:.8rem; }
        .card-delete-corner-btn { position: absolute; top: 15px; right: 15px; font-size: 1.4rem; font-weight: bold; line-height: 1; color: #dc3545; text-decoration: none; cursor: pointer; border: none; background: none; opacity: 0.6; transition: opacity 0.2s; }
        .card-delete-corner-btn:hover { opacity: 1; color: #bd2130; }
        /* Read Only Mode CSS toggles */
        .card-locked-state .freq-btn:not(.active) { display: none; }
        .card-locked-state .freq-btn.active { pointer-events: none; border-color: #198754; background: #198754; color: #fff; border-radius: 6px !important; }
        .card-locked-state .form-select { border: none; background-image: none; padding-left: 0; pointer-events: none; font-weight: 500; }
        .card-locked-state .btn-item-remove { display: none !important; }
    </style>
</head>
<body>
                    ...
                    <div class="d-flex flex-wrap justify-content-between align-items-start gap-2 mb-3 pe-4">
                        <div>
                            <span class="badge <?php echo $statusClass; ?> fs-6 me-2 status-label-text">
                                <?php echo strtoupper($rec['status']); ?>
                            </span>
...
function saveCardSettings(recurringID) {
    ...
    .then(r => r.json())
    .then(data => {
        if (!data.success) { alert('Error updating baseline rules: ' + (data.error || 'Unknown')); return; }
        ...
        Promise.all(itemPromises).then(() => {
            ...
            if (nextEl && d.nextDeliveryDate) {
                ...
                nextEl.textContent = dt.toLocaleDateString('en-PH', { year:'numeric', month:'short', day:'numeric' });
            }
...
function updateRecurringState(recurringID, payload, card) {
    const body = new URLSearchParams({ action: 'update_recurring', recurringID });
    ...
    .then(data => {
        ...
        card.querySelectorAll('.status-label-text').forEach(b => {
            const statusClass = { active:'status-badge-active', paused:'status-badge-paused', cancelled:'status-badge-cancelled' }[d.status] || '';
